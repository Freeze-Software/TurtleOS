...
int window_draw_rect(int window, int x, int y, int w, int h, uint32_t color) {
	...
	for (int yy = y; yy < (y + h); yy++) {
		for (int xx = x; xx < (x + w); xx++) {
			w2->framebuffer[yy * w2->w + xx] = color;	
		}
	}
	return 0;
}
        ...
        for (int col = 0; col < 8; col++) {
            ...
            for (int dy = 0; dy < size; dy++) {
                for (int dx = 0; dx < size; dx++) {
                    ...
                    w->framebuffer[py * w->w + px] = pixel_color;
                }
            }
        }
    }
...
void test_window_task() {
	while (1) {
		int x = (100 - (8 / 2));
		int y = (100 - (16 / 2));
		window_printf(test_window_id, x, y, 1, 0xFFFFFF, 0x000000, "%c", kb_key);
	}
}
...
void wm_render() {
    ...
    sprintf(dbg, "free=%d backbuffer=%x sw=%d sh=%d size=%d\n", stats.free, (uint32_t)backbuffer, sw, sh, sw * sh);
    serial_write(dbg);
    ...
    while (wm_running) {
	   ...
	   serial_write("clear\n");
	    ...
	    serial_write("Draw all windows");
            draw_all_windows();
	    serial_write("Draw taskbar");
	   draw_taskbar();
	    serial_write("Update drag");
